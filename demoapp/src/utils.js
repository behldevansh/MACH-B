...
export const addLogEntrytoDatabase = async (logData) => {
  try {
    ...
    if (res.message == "Success") {
      alert("Log Added");
      // Also trigger IPFS and blockchain storage
      await addLogsToIPFSandBlockchain();
    }
    else throw "Error";
  } catch (e) {
...
export const addNumberofLogstoDatabase = async (number, campLocation) => {
  try {
    // Generate complete CSV content with header
    let csvContent = "camp,timestamp,source,destination,user,device,eventType,eventDescription,eventSeverity,mlRiskScore\n";
    
    // Generate log rows
    for (let i = 0; i < number; i++) {
      csvContent += `${campLocation},${new Date().toISOString()},${randomIP()},${randomIP()},user${Math.floor(Math.random() * 100)},Device${Math.floor(Math.random() * 50)},${randomElementFromArray(["auth-failed","network-connected"])},"Security event ${i}",${randomElementFromArray(["informational","warning"])},${Math.random().toFixed(2)}\n`;
    }
    ...
    const response = await fetch(`${BACKEND_BASE_URL}/addLogs`, {
      ...
      body: JSON.stringify({
        ...
        csvHeader: "camp,timestamp,source,destination,user,device,eventType,eventDescription,eventSeverity,mlRiskScore"
      }),
    });
    const res = await response.json();
    if (res.message == "Success") {
